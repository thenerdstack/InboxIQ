    ...
    <>
      ...
      <SectionMain>
        ...
        <CardBox>
          ...
          <>
            <p className={'block font-bold mb-2'}>Email_accounts User</p>
            <CardBox
              className='mb-6 border border-gray-300 rounded overflow-hidden'
              hasTable
            >
              <div className='overflow-x-auto'>
                <table>
                  ...
                  <tbody>
                    {users.email_accounts_user &&
                      Array.isArray(users.email_accounts_user) &&
                      users.email_accounts_user.map((item: any) => (
                        <tr
                          key={item.id}
                          onClick={() =>
                            router.push(
                              `/email_accounts/email_accounts-view/?id=${item.id}`,
                            )
                          }
                        >
                          <td data-label='email_provider'>
                            {item.email_provider}
                          </td>

                          <td data-label='email_address'>
                            {item.email_address}
                          </td>
              ...
              {!users?.email_accounts_user?.length && (
                <div className={'text-center py-4'}>No data</div>
              )}
            </CardBox>
          </>
