...
import {
  ...
  deleteItem,
  setRefetch,
} from '../../stores/email_accounts/email_accountsSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureEmail_accountsCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';
import { dataGridStyles } from '../../styles';

import ListEmail_accounts from './ListEmail_accounts';

const perPage = 10;

const TableSampleEmail_accounts = ({
  filterItems,
  setFilterItems,
  filters,
}) => {
  ...
  const {
    ...
    loading,
    count,
    notify: email_accountsNotify,
    refetch,
  } = useAppSelector((state) => state.email_accounts);
  ...
  useEffect(() => {
    if (email_accountsNotify.showNotification) {
      notify(
        email_accountsNotify.typeNotification,
        email_accountsNotify.textNotification,
      );
    }
  }, [email_accountsNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/email_accounts/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/email_accounts/email_accounts-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {email_accounts && Array.isArray(email_accounts) && (
        <ListEmail_accounts
...
export default TableSampleEmail_accounts;
